...
class NeuronLayer(object):
    ...
    # Método constructor para la clase Capa de Red Neuronal que recibe un entero con el número de neuronas de la capa y
    ...
    def __init__(self, neurons: int, n_weights: int, weights=None, bias=None, lrate=0.1, ac_function=Sigmoid()):
        if type(neurons) != int:
            ...
            raise ValueError("El número de neuronas neurons debe ser un número entero positivo")
        elif type(weights) is not dict or type(bias) is not dict:
            raise ValueError("El input weights y bias deben ser diccionarios con las listas de pesos y bias de cada "+
