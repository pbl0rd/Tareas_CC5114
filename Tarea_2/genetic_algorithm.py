...
class GENALG(object):
    ...
    def __init__(self, pop_sz_0: int, fit_fn, cr_genes, cr_indv, indv_chars, term_cond, mut_rate=0.2, elitism_rate=0.0):
        ...
        # atributo con la cantidad de genes de un individuo
    ...
    def crossover(self, indv1, indv2):
        vals1 = list(indv1.values())  # lista con valores de los genes del padre 1
        vals2 = list(indv2.values())  # lista con valores de los genes del padre 2
        cut = np.random.randint(1, self.__num_genes)  # elegimos un punto de corte al azar
        # generamos los valores del nuevo individuo concatenando los valores del padre 1 hasta el punto de corte con los
        # del padre 2 desde el punto de corte
        new_vals = vals1[:cut] + vals2[cut:]
        keys = list(indv1.keys())
        new_ind = dict(zip(keys, new_vals)) # generamos el nuevo individuo
        return new_ind  # retornamos el nuevo individuo

    # Método para aplicar la operación mutation a 1 individuo y generar un nuevo individuo.
    def mutation(self, indv):
        vals = list(indv.values())  # lista con valores de los genes del individuo
        new_vals = []  # lista vacía para guardar los nuevos valores
        for i in range(self.__num_genes):  # recorremos los genes del individuo
            # si al extraer un numero aleatorio este es mayor que la tasa de mutación no se aplica la mutación
            if np.random.rand() > self.__mut_rate:
                new_vals.append(vals[i])
            else: # en caso contrario generamos un nuevo valor mediante la función de genes con las características
                # correspondientes a un gen de ese posición.
                cond = True
                while cond:
                    new_val = self.__cr_genes(self.__indv_chars[i]['gene_type'], self.__indv_chars[i]['fact_range'])
                    if new_val != vals[i]: # nos aseguramos de que la mutación cambie el valor actual del gen
                        cond = False
        ...
        if self.__term_cond['type'] == 'iterations':  # Si el criterio de parada es un número de iteraciones máximo
            ...
            while iters < self.__term_cond['iters']:  # iteramos hasta llegar al número de iteraciones máximo
                ...
                for i in range(self.__elitism, self.__pop_sz):
                    ...
                    new_pop[i] = child  # añadimos al hijo a la nueva población
